...
def main(page: ft.Page):
    ...
    def process_youtube(_):
        ...
        # ./cacheフォルダに音声ファイルを保存
        cache_dir = "./cache/"
        audio_path, error = audio_processor.download_youtube_audio(youtube_url.value, cache_dir)
    ...
    def pick_files_result(e: ft.FilePickerResultEvent):
        ...
        if e.files:
            ...
            # 文字起こしを実行
            result, error = audio_processor.transcribe_audio(selected_file.path, prompt_text.value)
            if error:
    ...
    select_button = ft.ElevatedButton(
        ...
        on_click=lambda _: file_picker.pick_files(
            allowed_extensions=["mp3", "wav", "m4a", "aac", "ogg", "flac", "opus"]  # 許可する音声ファイルの拡張子
        )
    )
